...
static AJ_Status PropGetHandler(AJ_Message* replyMsg, uint32_t propId, char* errorName, size_t errorNameBufSize, char* errorMessage, size_t errorMessageBufSize, void* context)
{
    /* Simulate that an error occurred while trying to read a property */
    strncpy(errorName, "GetPropertyError", errorNameBufSize);
    strncpy(errorMessage, "Error getting property", errorMessageBufSize);
    return AJ_ERR_UNEXPECTED;
}


static AJ_Status PropSetHandler(AJ_Message* replyMsg, uint32_t propId, char* errorName, size_t errorNameBufSize, char* errorMessage, size_t errorMessageBufSize, void* context)
{
    /* Simulate that an error occurred while trying to apply a property */
    strncpy(errorName, "SetPropertyError", errorNameBufSize);
    strncpy(errorMessage, "Error setting property", errorMessageBufSize);
    return AJ_ERR_UNEXPECTED;
}
...
#endif
{
    ...
    while (TRUE) {
            ...
            case AJ_METHOD_ACCEPT_SESSION:
                {
                    ...
                    if (status != AJ_OK) {
                        AJ_AlwaysPrintf(("Failed to unmarshal message arguments: %s\n", AJ_StatusText(status)));
                        break;
                    }
                    if (port == ServicePort) {
            ...
            case AJ_SIGNAL_SESSION_LOST_WITH_REASON:
                {
                    ...
                    if (CancelAdvertiseName) {
                        ...
                        if (status != AJ_OK) {
                            AJ_AlwaysPrintf(("Failed to advertise %s: %s\n", ServiceName, AJ_StatusText(status)));
                        }
                    }
         ...
         */
        AJ_Status closeMsgStatus = AJ_CloseMsg(&msg);
        if (closeMsgStatus != AJ_OK) {
            AJ_AlwaysPrintf(("Failed to close message: %s\n", AJ_StatusText(closeMsgStatus)));
        }
