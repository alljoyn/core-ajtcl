...
#endif
{
    ...
    while (TRUE) {
        ...
        if (status == AJ_OK) {
            ...
            case APP_GET_PROP:
                status = AJ_BusPropGetWithError(&msg, PropGetHandler, "GetPropertyError", "Error getting property", NULL);
                AJ_AlwaysPrintf(("AJ_BusPropGetWithError() returned %s\n", AJ_StatusText(status)));
                break;

            case APP_GET_ALL_PROPS:
                status = AJ_BusPropGetAllWithError(&msg, PropGetHandler, "GetPropertyError", "Error getting property", NULL);
                AJ_AlwaysPrintf(("AJ_BusPropGetAllWithError() returned %s\n", AJ_StatusText(status)));
                break;

            case APP_SET_PROP:
                status = AJ_BusPropSetWithError(&msg, PropSetHandler, "SetPropertyError", "Error setting property", NULL);
    ...
    AJ_WarnPrintf(("Exiting prop_with_error_test_svc with status %s\n", AJ_StatusText(status)));
