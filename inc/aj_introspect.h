 ...
 *
 * @param[in]  replyMsg             The message to marshal the reply into. Assumes the message header is already set by a previous call to AJ_MarshalReplyMsg()
 * @param[in] iface                 The interface name (obtained from the request) whose properties are to be marshalled
 * @param[in]  callback             The function called to request the application to marshal each property value
 * @param[in]  callbackWithError    The function called to request the application to marshal each property value, which contains additional parameters to return a possible error
 ...
 * @param[in]  context              A caller provided context that is passed into the callback function
 *
 * @return                          Return AJ_Status
 */
AJ_EXPORT
