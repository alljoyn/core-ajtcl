...
#include "aj_cert.h" // For HostU32ToBigEndianU8; will go away when feature/security2.0 merges into master
...
AJ_Status AJ_PeerAuthenticate(AJ_BusAttachment* bus, const char* peerName, AJ_PeerAuthenticateCallback callback, void* cbContext)
{
     ...
     */
    AJ_ConversationHash_Update_Message(&authContext, CONVERSATION_V4, &msg, 1);
...
static AJ_Status ExchangeSuites(AJ_Message* msg)
{
    ...
    AJ_ConversationHash_Update_Message(&authContext, CONVERSATION_V4, &call, 1);
