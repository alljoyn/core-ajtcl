...
static AJ_Status SendHeader(uint8_t flags)
{
    ...
    return AJ_ARDP_UDP_Send(conn->context, (uint8_t*) &buf32, ARDP_HEADER_SIZE, &sent);
}
...
static AJ_Status SendSyn(uint16_t dataLen)
{
    ...
    *((uint16_t*) (txbuf + SYN_RSRV_OFFSET)) = 0;
    ...
    return AJ_ARDP_UDP_Send(conn->context, (uint8_t*) &conn->snd.buf[0].data[0], ARDP_SYN_HEADER_SIZE + dataLen, &sent);
}
...
static AJ_Status ConnectTimerHandler()
{
    ...
    if (conn->connectTimer.retry > 1) {
        ...
        status = AJ_ARDP_UDP_Send(conn->context, (uint8_t*) conn->snd.buf[0].data, len, &sent);
    ...
    do {
        if ((msElapsed >= timeout) && (timer->retry > UDP_MIN_DATA_RETRIES)) {
            ...
            *((uint32_t*) (txbuf + ACKNXT_OFFSET)) = htonl(conn->snd.UNA);
            ...
            status =  AJ_ARDP_UDP_Send(conn->context, (uint8_t*) sBuf->data, len, &sent);
...
static AJ_Status ArdpMachine(struct ArdpSeg* seg, uint8_t* rxBuf, uint16_t len, uint8_t** dataBuf, uint16_t* dataLen, void**rxContext)
{
    ...
    AJ_InfoPrintf(("ArdpMachine(handle=%p, conn=%p, seg=%p, buf=%p, len=%d)\n", conn, seg, rxBuf, len));
    ...
    case SYN_SENT:
        {
            ...
            /* No copy here: the assumption is that at this point the Read buffer of the caller is wide open */
            if (status == AJ_OK) {
                *dataBuf = rxBuf + (2 * seg->HLEN);
                *dataLen = seg->DLEN;
                conn->rcv.buf[0].seq = seg->SEQ;
                *rxContext = (void*) &conn->rcv.buf[0];
            }
            ...
            if (status == AJ_OK) {
                ...
                if (seg->FLG & ARDP_FLAG_NUL) {
                    ...
                    if (SEQ32_LET((conn->rcv.CUR + 1), seg->SEQ)) {
                        ...
                        assert(conn->rcv.buf[index].fcnt == 0);
...
AJ_Status ARDP_Recv(uint8_t* rxBuf, uint16_t len, uint8_t** dataBuf, uint16_t* dataLen, void** rxContext)
{
    ...
    if (rxBuf != NULL) {
        ...
        *dataLen = 0;
        ...
        //TODO: assert(*rxContext == NULL);

        status = RecvValidateSegment(rxBuf, len, &seg);
        if (status == AJ_OK) {
            status = ArdpMachine(&seg, rxBuf, len, dataBuf, dataLen, rxContext);
        }
    }
    ...
    if (status != AJ_OK) {
        AJ_ErrPrintf(("ARDP_Recv(): return %s (0x%x)\n", AJ_StatusText(status), status));
    }
    return status;
}
...
AJ_Status ARDP_Send(uint8_t* txBuf, uint16_t len) {
    ...
    do {
        ...
        status =  AJ_ARDP_UDP_Send(conn->context, (uint8_t*) sBuf->data, ARDP_HEADER_SIZE + dataLen, &sent);
...
AJ_Status ARDP_Connect(uint8_t* data, uint16_t dataLen, void* context)
{
    ...
    assert(dataLen < (UDP_SEGBMAX - ARDP_SYN_HEADER_SIZE));
        ...
        if (status == AJ_ERR_ARDP_BACKPRESSURE && NetSock != NULL) {
            do {
                ...
                if (status != AJ_OK && status != AJ_ERR_TIMEOUT) {
                    ...
                    AJ_ErrPrintf(("AJ_ARDP_Send: AJ_ARDP_UDP_Recv returns %s\n", AJ_StatusText(status)));
                    return AJ_ERR_WRITE;
                }
...
AJ_Status AJ_ARDP_Recv(AJ_IOBuffer* rxBuf, uint32_t len, uint32_t timeout)
{
    ...
    if (UDP_Recv_State.rxContext != NULL) {
        ...
        if (UDP_Recv_State.datalen <= rx) {
            ...
            if (len == 0) {
                // no more room to recv; run the state machine and call it a day
                return ARDP_Recv(NULL, 0, NULL, NULL, NULL);
            }
        } else {
            if (rx != 0) {
                ...
                // save where we were!
                UDP_Recv_State.recved_buffer += rx;
                UDP_Recv_State.datalen -= rx;
                len -= rx;
            }

            // we still have unconsumed data so we can't recv yet
            return ARDP_Recv(NULL, 0, NULL, NULL, NULL);
        }
    }

    // need to know when we're finished
    AJ_InitTimer(&end);
    ...
    do {
        ...
        status = AJ_ARDP_UDP_Recv(rxBuf->context, buf, sizeof(buf), timeout2, &received);

        switch (status) {
        case AJ_ERR_TIMEOUT:
            status = ARDP_Recv(NULL, 0, NULL, NULL, NULL);
            break;
        ...
        case AJ_OK:
            ...
            case AJ_ERR_ARDP_RECV_EXPIRED:
                ...
                AJ_IO_BUF_RESET(rxBuf);
                // intentionally fall through because there might be a new message starting!
                if (UDP_Recv_State.recved_buffer == NULL) {
                    // nothing else to copy; simply return
                    return status;
                }
            ...
            case AJ_OK:
                ...
                // else this was an ARDP packet that we must ignore
                break;
