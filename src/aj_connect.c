...
static uint8_t ServiceIsEqual(AJ_Service* A, AJ_Service* B) {
    ...
    /**
       A service is equal if at least one of its address/port pairs match
...
void AJ_AddRoutingNodeToResponseList(AJ_Service* service)
{
    ...
    AJ_InfoPrintf(("Added RN %x:%x:%x:%x port = %d, addrtype = %d\n", service->ipv6[0], service->ipv6[1], service->ipv6[2], service->ipv6[3], service->ipv6port, service->addrTypes));
}
