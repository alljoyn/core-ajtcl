...
void AJ_AddRoutingNodeToResponseList(AJ_Service* service)
{
    ...
    if (service->addrTypes & AJ_ADDR_TCP4) {
        if (inet_ntop(AF_INET, &(service->ipv4), ipStr, INET_ADDRSTRLEN) != NULL) {
            AJ_InfoPrintf(("Added RN %s (pv = %d, port = %d, priority = %d) to list (slot %d of %d)\n",
                           ipStr,
                           service->pv, service->ipv4port, service->priority, candidate, RNResponseListIndex));
        }
    }
    if (service->addrTypes & AJ_ADDR_UDP4) {
        if (inet_ntop(AF_INET, &(service->ipv4Udp), ipStr, INET_ADDRSTRLEN) != NULL) {
            AJ_InfoPrintf(("Added RN %s (pv = %d, port = %d, priority = %d) to list (slot %d of %d)\n",
                           ipStr,
                           service->pv, service->ipv4portUdp, service->priority, candidate, RNResponseListIndex));
        }
    }
    if (service->addrTypes & AJ_ADDR_UDP6) {
        if (inet_ntop(AF_INET6, &(service->ipv6Udp), ipStr, INET6_ADDRSTRLEN) != NULL) {
            AJ_InfoPrintf(("Added RN %s (pv = %d, port = %d, priority = %d) to list (slot %d of %d)\n",
                           ipStr,
...
AJ_Status AJ_SelectRoutingNodeFromResponseList(AJ_Service* service)
{
    ...
    if (service->addrTypes & AJ_ADDR_TCP4) {
        if (inet_ntop(AF_INET, &(service->ipv4), ipStr, INET_ADDRSTRLEN) != NULL) {
            AJ_InfoPrintf(("Selected routing node %s (pv = %d, port = %d, priority = %d) out of %d responses in the list.\n",
                           ipStr, service->pv, service->ipv4port, service->priority, RNResponseListIndex));
        }
    }
    if (service->addrTypes & AJ_ADDR_UDP4) {
        if (inet_ntop(AF_INET, &(service->ipv4Udp), ipStr, INET_ADDRSTRLEN) != NULL) {
            AJ_InfoPrintf(("Selected routing node %s (pv = %d, port = %d, priority = %d) out of %d responses in the list.\n",
                           ipStr, service->pv, service->ipv4portUdp, service->priority, RNResponseListIndex));
        }
    }
    if (service->addrTypes & AJ_ADDR_UDP6) {
        if (inet_ntop(AF_INET6, &(service->ipv6Udp), ipStr, INET6_ADDRSTRLEN) != NULL) {
            AJ_InfoPrintf(("Selected routing node %s (pv = %d, port = %d, priority = %d) out of %d responses in the list.\n",
                           ipStr, service->pv, service->ipv6portUdp, service->priority, RNResponseListIndex));
        }
    }
    return AJ_OK;
}
