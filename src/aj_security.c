...
AJ_Status AJ_SecurityClaimMethod(AJ_Message* msg, AJ_Message* reply)
{
    ...
    /* Calling "StartManagmenet" remotely is impossible before claiming, so it has to be done here. */
...
Exit:
    ...
    if (AJ_OK == status) {
        ...
        /* Result ignored, because this call only fails if managamenet did not start, which is the end state we need. */
        AJ_SecurityEndManagement(msg->bus);
