...
boolean_t validate_256(digit256_tc a, digit256_tc modulus)
{
    ...
    valid = ((valid & (fpiszero_p256(t1) ^ 1)) != 0) ? B_TRUE : B_FALSE;  /* valid = B_TRUE if a < modulus (use & instead of && for constant-time execution) */
    ...
    digit256_t a)
{
    ...
    return ((borrow ^ 1) != 0) ? B_TRUE : B_FALSE;
}
...
boolean_t fpequal_p256(digit256_tc f1, digit256_tc f2)
{
    ...
    equal = ((~((((sdigit_t)temp) >> (RADIX_BITS - 1)) | (-((sdigit_t)temp) >> (RADIX_BITS - 1))) & 1) != 0) ? B_TRUE : B_FALSE;
