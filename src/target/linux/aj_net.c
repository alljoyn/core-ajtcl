...
static uint8_t sendToBroadcast(int sock, uint16_t port, AJ_IOBuffer* buf, size_t tx)
{
    ...
    if (0 == getsockopt(sock, SOL_SOCKET, SO_BROADCAST, &optval, &optlen)) {
        if (optval == 0) {
            AJ_WarnPrintf(("sendToBroadcast(): sock %d has no broadcast enabled.\n", sock));
            return FALSE;
        }
    }
...
static AJ_Status RewriteSenderInfo(AJ_IOBuffer* buf, struct sockaddr* saddr)
{
    ...
    if (saddr->sa_family == AF_INET) {
        upcToUse = upcv4;
        ipToUse = ipv4;
        status = AJ_IntToString((int32_t)htons(((struct sockaddr_in*)saddr)->sin_port), upcStr, sizeof(upcStr));
        if (status != AJ_OK) {
            return AJ_ERR_WRITE;
        }
        ...
        upcToUse = upcv6;
        ipToUse = ipv6;
        status = AJ_IntToString((int32_t)htons(((struct sockaddr_in6*)saddr)->sin6_port), upcStr, sizeof(upcStr));
        if (status != AJ_OK) {
            return AJ_ERR_WRITE;
        }
    ...
    if (context->mDnsRecvSock != INVALID_SOCKET && FD_ISSET(context->mDnsRecvSock, &fds)) {
        ...
        if (rx) {
            ...
            if (ret == -1) {
                ...
                AJ_InfoPrintf(("AJ_Net_RecvFrom(): recv'd %d from mDNS over IPv4\n", (int) ret));
...
static int MCastUp4(const char group[], uint16_t port)
{
    ...
    if (r != 0) {
        AJ_ErrPrintf(("MCastUp4(): setsockopt(SO_BROADCAST) failed. errno=\"%s\"\n", strerror(errno)));
    }
    ...
    if (ret < 0) {
        ...
        if (r != 0) {
            AJ_ErrPrintf(("MCastUp4(): broadcast and multicast failed, status=AJ_ERR_READ\n", strerror(errno)));
            goto ExitError;
        }
    }
...
static int MCastUp6(const char* group, uint16_t port)
{
    ...
    if (ret < 0) {
        AJ_ErrPrintf(("MCastUp6(): setsockopt(IPV6_JOIN_GROUP) failed. errno=\"%s\", status=AJ_ERR_READ\n", strerror(errno)));
        goto ExitError;
    }
...
static int MDns6RecvUp()
{
    ...
    if (recvSock == INVALID_SOCKET) {
        AJ_ErrPrintf(("%s(): socket() fails. status=AJ_ERR_READ\n", __FUNCTION__));
        goto ExitError;
    }
...
AJ_Status AJ_Net_MCastUp(AJ_MCastSocket* mcastSock)
{
    ...
    do {
        ...
        if (getsockname(mCastContext.mDnsRecvSock, (struct sockaddr*) &addrBuf, &addrLen) == -1) {
    ...
    if ((statusIPv4 != AJ_OK) && (mCastContext.mDnsRecvSock != INVALID_SOCKET)) {
        close(mCastContext.mDnsRecvSock);
        mCastContext.mDnsRecvSock = INVALID_SOCKET;
    }
    ...
    do {
        ...
        if (getsockname(mCastContext.mDns6RecvSock, (struct sockaddr*) &addrBuf, &addrLen) == -1) {
    ...
    if ((statusIPv6 != AJ_OK) && (mCastContext.mDns6RecvSock != INVALID_SOCKET)) {
        close(mCastContext.mDns6RecvSock);
        mCastContext.mDns6RecvSock = INVALID_SOCKET;
    }
...
static int AJ_Net_ARDP_PrepareConnectIPv4(const AJ_Service* service, struct sockaddr_storage* destAddr)
{
    ...
    AJ_InfoPrintf(("%s(): Connect to \"%s:%u\"\n", __FUNCTION__, inet_ntoa(sa->sin_addr), service->ipv4portUdp));
    return udpSock;
}

static int AJ_Net_ARDP_PrepareConnectIPv6(const AJ_Service* service, struct sockaddr_storage* destAddr)
{
    int udpSock = INVALID_SOCKET;
...
static AJ_Status AJ_Net_ARDP_DoConnect(AJ_BusAttachment* bus, const AJ_Service* service,
                                       int udpSock, const struct sockaddr_storage* destAddr, socklen_t destAddrSize)
{
    ...
    int ret = connect(udpSock, (struct sockaddr*)destAddr, destAddrSize);
    ...
    if (ret == 0) {
        ...
        AJ_ErrPrintf(("%s(): Error connecting\n", __FUNCTION__));
        perror("connect");
        return AJ_ERR_CONNECT;
    }
...
static AJ_Status AJ_Net_ARDP_Connect(AJ_BusAttachment* bus, const AJ_Service* service)
{
    ...
    interruptFd = eventfd(0, O_NONBLOCK);  // Use O_NONBLOCK instead of EFD_NONBLOCK due to bug in OpenWrt's uCLibc
    if (interruptFd < 0) {
        AJ_ErrPrintf(("%s(): failed to create interrupt event\n", __FUNCTION__));
        goto ConnectError;
    }
