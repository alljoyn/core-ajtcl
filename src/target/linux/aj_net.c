...
static uint8_t sendToBroadcast(int sock, uint16_t port, AJ_IOBuffer* buf, size_t tx)
{
    ...
    while (addr != NULL) {
        if (addr->ifa_addr != NULL &&
            ...
            struct sockaddr* sin_bcast =  addr->ifa_ifu.ifu_broadaddr;
            void*src;
            if ((NULL != sin_bcast) || (AF_INET6 == addr->ifa_addr->sa_family)) {
                if (NULL == sin_bcast) {
                    sin_bcast = addr->ifa_addr;
                }
                if (AF_INET == sin_bcast->sa_family) {
                    ((struct sockaddr_in*)sin_bcast)->sin_port = htons(port);
                    src = &((struct sockaddr_in*)sin_bcast)->sin_addr;
                } else if (AF_INET6 == sin_bcast->sa_family) {
                    ((struct sockaddr_in6*)sin_bcast)->sin6_port = htons(port);
                    src = &((struct sockaddr_in6*)sin_bcast)->sin6_addr;
                }
                inet_ntop(addr->ifa_addr->sa_family, src, addrbuf, sizeof(addrbuf));
                AJ_InfoPrintf(("sendToBroadcast: sending to bcast addr %s\n", addrbuf));
                if (buf->flags & AJ_IO_BUF_MDNS) {
                    if (RewriteSenderInfo(buf, sin_bcast) != AJ_OK) {
                ...
                AJ_InfoPrintf(("sendToBroadcast: NOT sending to bcast addr %s\n", addrbuf));
            }
...
AJ_Status AJ_Net_MCastUp(AJ_MCastSocket* mcastSock)
{
    ...
    if ((mCastContext.mDnsRecvSock == INVALID_SOCKET) && (mCastContext.mDns6RecvSock == INVALID_SOCKET)) {
    ...
    if (INVALID_SOCKET != mCastContext.mDnsRecvSock) {
...

    {
        ...
        if (NULL != inet_ntop(mCastContext.mDnsRecvAddr.sin6_family, &mCastContext.mDnsRecvAddr,
                              printAddrbuf, sizeof(printAddrbuf))) {
            AJ_InfoPrintf(("AJ_Net_MCastUp(): mDNS recv on %s-%d\n", printAddrbuf, ntohs(mCastContext.mDnsRecvAddr.sin6_port)));
        }
    }
