...
AJ_Status AJ_IntToString(int32_t val, char* buf, size_t buflen)
{
    ...
    int c = _snprintf_s(buf, buflen, buflen, "%d", val);
    if (c <= 0 || c > buflen) {
...
{
    AJ_Status status = AJ_OK;
    int c = _snprintf_s(buf, buflen, buflen, "%u.%u.%u.%u", (addr & 0xFF000000) >> 24, (addr & 0x00FF0000) >> 16, (addr & 0x0000FF00) >> 8, (addr & 0x000000FF));
    if (c <= 0 || c > buflen) {
        status = AJ_ERR_RESOURCES;
    }
    return status;
