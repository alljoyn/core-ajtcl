    ...
    if (status == AJ_OK) {
        ...
        if (op == AJ_PROP_GET) {
            ...
            if (strcmp(sig, variant) == 0) {
                status = cb->SetWithError ? cb->SetWithError(&reply, propId, errorName, ERROR_NAME_BUF_SIZE, errorMessage, ERROR_MSG_BUF_SIZE, cb->context)
...
static AJ_Status PropAccessAll(AJ_Message* msg, PropCallback* cb)
{
    ...
    if (status == AJ_OK) {
        status = cb->GetWithError ? AJ_MarshalAllPropertiesArgs(&reply, iface, NULL, cb->GetWithError, errorName, ERROR_NAME_BUF_SIZE, errorMessage, ERROR_MSG_BUF_SIZE, cb->context)
