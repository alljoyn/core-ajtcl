...
void AJ_ConversationHash_Update_Message(AJ_AuthenticationContext* ctx, uint32_t conversationVersion, AJ_Message* msg, uint8_t isMarshaledMessage)
{
    ...
    if (HASH_MSG_MARSHALED == isMarshaledMessage) {
        ...
        /* Reset the message to make sure all arguments have been received and placed into the I/O buffer.
         * If the message hasn't been unmarshaled yet, some data may still be waiting in the networking layer. */
