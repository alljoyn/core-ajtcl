...
static AJ_Status AJ_PermissionRuleMarshal(const AJ_PermissionRule* head, AJ_Message* msg, uint8_t isTemplate)
{
    ...
    while (head) {
        ...
        if (isTemplate) {
            /* Decrease securityLevel value by 1 to match values used by SCL */
            status = AJ_MarshalArgs(msg, "ssy", head->obj, head->ifn, ((head->securityLevel == NOT_SET) ? (PRIVILEGED - 1) : (head->securityLevel - 1)));
        } else {
